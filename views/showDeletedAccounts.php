...
try {
    ...
    $query = "SELECT isShown,a.date as datee, totalPrice as total, a.id_account as accountId, p.name as patientName, p.surname as patientSurname, e.name as doctorName, e.surname as doctorSurname FROM accounts a INNER JOIN patients p ON p.id_patient = a.id_patient INNER JOIN employees e ON e.id_employee = a.id_employee WHERE isShown = 0";
    ...
    foreach ($result as $r) {
        ...
        // BUTTONI I CHECKED
        $output .= "<div class='options-vab head-tcss fc'><button class='viewButtonAccDeleted crudbtns fc' data-id='$r->accountId'>$viewsvg</button>";
        ...
        if (in_array($_SESSION["id_role"], [2, 3, 5])) {
            $cek = "<form class='form'>
    ...
    if ($pageCount > 1) {
        ...
        $output .= "<li><a data-page='1' class='btnAccountPagination btnPagination'>Prva</a></li>";
        for ($i = 1; $i <= $pageCount; $i++) {
            $class = $page == $i ? 'btn-activeAccountPagination' : "";
            $output .= "<li class='" . $class . "'><a data-page='$i' class='btnAccountPagination'>" . $i . "</a></li>";
        }
        $output .= "<li><a data-page='$pageCount' class='btnAccountPagination btnPagination'>Poslednja</a></li>";
        $output .= "</ul></div>";
    }
    echo $output;
