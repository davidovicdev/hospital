...
?>
    ...
    $ff = $ff[0];
    ?>
    <div class="modal modal-w" id="modal10">
        ...
        <div class="modal-body scroll" id="modal10-body">
            ...
            foreach ($result as $r) {
                ...
                $outputmodal .= "</select></div><div class='therapy-vab head-tcss fl'><ul>";
                ...
                if ($therapiesRowCount > 0) {
                    $outputmodal .= "<li><button class='btn btnTherapyAccount' data-idAccount='$resultTherapies->accountId'>$resultTherapies->therapy</button></li>"; //! OVO NE RADI MORA SE MENJA
                } else {
                    $outputmodal .= "<li>Nema terapije</li>";
                }
                $outputmodal .= "</ul></div><div class='appointment-vab head-tcss custom-select fc'><select class='' id='bloodsSelectModal'>";
                // APPOINTMENT
                ...
                $outputmodal .= "<button class='duplicateButtonAccEmployee crudbtns fc' data-id='$r->accountId'>$duplicatesvg</button>";
                $outputmodal .= "<button class='printButtonAccEmployee crudbtns fc mr2' data-id='$r->accountId'>$printsvg</button>";
                if (in_array($_SESSION["id_role"], [1, 3])) {
                    $outputmodal .= "<button class='updateButtonAccEmployee crudbtns fc mr2' data-id='$r->accountId'>$editsvg</button>";
                    $outputmodal .= "<button class='deleteButtonAccEmployee crudbtns fc' data-id='$r->accountId'>$deletesvg</button>";
                }
                $outputmodal .= "</div>";
                // CHECKED
                ...
                if (in_array($_SESSION["id_role"], [2, 5])) {
                    $cek = "<form class='form'>
                            <div class='inputGroup'>
                              <input id='option' data-id='$r->accountId' name='option' type='checkbox' class='option checkboxCheck'/>
                              <label for='option'>Čekiraj</label>
                            </div>
            ...
            if ($result) {
                ...
                foreach ($result as $r) {
                    ...
                    $outputmodal .= "$subAccountId</div><div class='namesur-vab fl head-tcss'>$r->patientName $r->patientSurname</div><div class='doctor-vab head-tcss fl'>$r->doctorName $r->doctorSurname</div><div class='blood-vab head-tcss custom-select'><select class='selectlist' id='bloodsSelectModal'><option>Vidi krv</option>";
                    ...
                    $resultTherapies = $resultTherapies->fetch();
                    if ($therapiesRowCount > 0) {
                        $outputmodal .= "<li><button class='btn btnTherapySubAccount' data-idSubAccount='$resultTherapies->accountId'>$resultTherapies->therapy</button></li>";
                    } else {
                        $outputmodal .= "<li>Nema terapije</li>";
                    }
                    $outputmodal .= "</ul></div><div class='appointment-vab head-tcss custom-select'><select class='' id='appointmentsSelectModal'>";
                    // APPOINTMENT
                    ...
                    $outputmodal .= "<button class='printButtonSubAcc crudbtns fc mr2' data-id='$r->subAccountId'>$printsvg</button>";
                    if (in_array($_SESSION["id_role"], [1, 3])) {
                        $outputmodal .= "<button class='updateButtonAcc1 crudbtns fc mr2' data-id='$r->subAccountId'>$editsvg</button>";
