...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    try {
        ...
        #var_dump($id);
        $query = "SELECT *,id_sub_account as subAccountId, a.date as datee, sa.totalPrice as total, a.id_account as accountId, p.name as patientName, p.surname as patientSurname, e.name as doctorName, e.surname as doctorSurname FROM sub_accounts sa INNER JOIN accounts a ON sa.id_account = a.id_account INNER JOIN patients p ON p.id_patient = a.id_patient INNER JOIN employees e ON e.id_employee = sa.id_employee WHERE sa.isShown = 1 AND e.id_employee=$id";
        $result = $con->query($query)->fetchAll();
        #var_dump($result);
        $outputmodal = "<div class='modal'><div class='modal-header'><h2>Podnalozi</h2><div class='close-button closeMe'>$deletesvg</div></div><div class='modal-body  scroll'>";
